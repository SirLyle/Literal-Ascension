...
fun World.playCarveSound(pos: BlockPos, entity: Entity) {
    val targetPoint = NetworkRegistry.TargetPoint(this.provider.dimension, pos.x.toDouble(), pos.y.toDouble(), pos.z.toDouble(), 16.0)
    LiteralAscension.packetHandler.sendToAllAround(MessagePlayCarveSound(pos, entity.entityId), targetPoint)
}

fun World.playLadderPickupSound(pos: BlockPos) {
    val targetPoint = NetworkRegistry.TargetPoint(this.provider.dimension, pos.x.toDouble(), pos.y.toDouble(), pos.z.toDouble(), 16.0)
